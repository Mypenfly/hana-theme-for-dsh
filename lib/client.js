...
function renderSeal(props) {
  ...
  return R.createElement(
    ...
    var CSS = [
      ...
      "",
      "  /* ── 字距寄存器 (tracking register) ────────────────────────────────",
      "     HanaAgent's letter-spacing register is 76 declarations and it is a CHROME",
      "     register: small grey labels take .02em, group headers .04-.06em, UPPERCASE",
      "     ones .06-.08em, and anything the user types, searches or reads as data is",
      "     EXPLICITLY zeroed (.settings-search-input { letter-spacing: 0 }). Measured",
      "     over its reading surface alone -- md-content / preview-markdown / md-* --",
      "     exactly ONE survives:",
      "",
      "         .previewPanelBody .preview-markdown thead th { letter-spacing: 0.08em }",
      "",
      "     The other 75 are keyed on class names, and DSH's equivalents are build",
      "     hashes -- the same wall as the radius ledger. But that is not the reason",
      "     they are not ported, because a documented 76-site register would be",
      "     portable in spirit. The reason is that DSH ALREADY HAS this register:",
      "     nine declarations, .035em on the trajectory kind tag through .08em on the",
      "     official wordmark, plus two EXPLICIT zeros on the sidebar's brand text.",
      "     Same rule, same band. Chrome tracking belongs to the host, and this theme",
      "     deliberately keeps chrome in its own voice (see the L3 header) --",
      "     re-tracking it would take apart the boundary between 'reading text becomes",
      "     a book' and everything else.",
      "",
      "     So the register here is the reading surface's, and it is a PAIR: a label",
      "     value for table headers, and an explicit zero for the data beneath them.",
      "     The zero changes no pixel today. It is written down because it is an",
      "     invariant that a mutation can break, which is worth more than a comment. */",
      "  --hana-track-label: 0.08em;",
      "  --hana-track-data: 0;",
      "",
      "  /* The link rule: HanaAgent colours the anchor's bottom BORDER at 35% of the",
      "     link ink and takes it to full on hover. See rule 3 below for why this is a",
      "     border and not a text-decoration, and why the hover has no ink swap. */",
      ...
      "/* 3. The link rule: HanaAgent's own treatment, which is a border-bottom and",
      ...
      "",
      "      An earlier revision of this theme used text-decoration-color instead, on",
      "      the recorded grounds that the harness paints the markdown anchor with a",
      "      deliberately TRANSPARENT 2px bottom border as an enlarged hit area, so",
      "      overriding it would shrink the click target. The border is real --",
      "      MarkdownText.module.css says Transparent hit-area padding -- but the",
      "      CONCLUSION was wrong: a hit area is the box's GEOMETRY, and colouring a",
      "      border that already exists does not narrow it. Changing the WIDTH is what",
      "      narrows it, and those are two different edits that were treated as one.",
      "",
      "      The width here is the reference's 1px rather than the harness's 2px. That",
      "      costs 1px of hit height at the bottom (the 3px left/right horizontal",
      "      enlargement, which is where a text link is actually clicked, is kept), and",
      "      it buys a hairline -- the same language as 「极方圆角 + 0.5px hairline」. 2px",
      "      under 14-16px serif body text is a visible bar, not a hairline.",
      "",
      "      The hover has to undo the harness's own, which is text-decoration:",
      "      underline: left alone it would stack a second line under ours. HanaAgent",
      "      ALSO swaps the ink for --link-hover there, and we cannot: that pair exists",
      "      only in its two midnight themes (珊瑚's links are its --accent), and this",
      "      palette set has no such token. So the hover step is carried by the rule",
      "      alone, 35% -> 100%. Recorded as a deviation, not papered over. */",
      "body[" + BODY_ATTR + "] [data-chat-flow-kind='assistant-step'] a {",
      "  text-decoration: none;",
      "  border-bottom: 1px solid var(--hana-link-rule);",
      "}",
      "body[" + BODY_ATTR + "] [data-chat-flow-kind='assistant-step'] a:hover,",
      "body[" + BODY_ATTR + "] [data-chat-flow-kind='assistant-step'] a:focus {",
      "  text-decoration: none;",
      ...
      "}",
      "",
      "/* 9. The tracking register, as a pair. A table header is a SHORT LABEL --",
      "      HanaAgent's reading surface tracks exactly one thing, and it is this one,",
      "      at this value. The cell beneath it is DATA, and data is what its register",
      "      explicitly zeros. See --hana-track-* above for the 76-declaration chrome",
      "      register and why porting it would be the wrong move rather than an",
      "      impossible one. */",
      "body[" + BODY_ATTR + "] [data-chat-flow-kind='assistant-step'] th {",
      "  letter-spacing: var(--hana-track-label);",
      "}",
      "body[" + BODY_ATTR + "] [data-chat-flow-kind='assistant-step'] td {",
