window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var PAPER = {
      ...
      "--dsw-alias-button-floating-fill": "#FDF8EF",
      ...
      "--dsw-alias-button-tool-bar-fill": "#FDF8EF",
    ...
    var CORAL = {
      ...
      "--dsw-alias-bg-layer-1": "#EFEEEB",
      "--dsw-alias-bg-layer-2": "#F5EADF",
      "--dsw-alias-bg-layer-3": "#EEE8E1",
      "--dsw-alias-bg-overlay": "#EFEEEB",
      ...
      "--dsw-alias-button-elevated-fill": "#EFEEEB",
      "--dsw-alias-button-floating-fill": "#EFEEEB",
      ...
      "--dsw-alias-button-tool-bar-fill": "#EFEEEB",
      ...
      "--dsw-specific-input-major": "#EFEEEB",
      "--dsw-specific-login-input": "#FFFBF3",
      "--dsw-specific-selector": "#F9EEE2",
      "--dsw-specific-menu": "#EFEEEB",
      "--dsw-specific-tip": "#F1E8DB",
...
function renderSeal(props) {
  ...
  return R.createElement(
    ...
    var CSS = [
      ...
      "  --hana-card-pad: 10px;",
      "  --hana-card-edge: 1px;",
      "  margin-inline: calc(-1 * (var(--hana-card-pad) + var(--hana-card-edge)));",
      "  padding: 8px var(--hana-card-pad);",
      "  background: var(--dsw-alias-bg-layer-1);",
      "  border: var(--hana-card-edge) solid var(--dsw-alias-border-l1);",
      "  border-radius: 6px;",
      "  box-shadow: 0 1px 2px var(--dsw-alias-bg-mask-drop);",
      "}",
      "/* A nested tool call keeps the card's own surface: two stacked cards would",
      "   hide the code panel the harness draws inside, which already has its own",
      ...
      "}",
      "",
      "/* THE COLLAPSED ONE-LINER IS A SEPARATE ELEMENT, and it was the gap. A tool",
      "   call has two presentations, and the first pass styled only one of them:",
      "",
      "     expanded    [data-turn-process-member] — the flow items inside the",
      "                 process, which the card above handles.",
      ...
      "",
      "   Both get the same language — a paper surface, a hairline all round, a",
      "   small radius. The collapsed one is a BAR rather than a card, because it",
      "   is a control that opens something and should not compete with what it",
      "   reveals. height: 33px is the harness's and is left alone; only the",
      "   paint and the horizontal box change. */",
      "body[" + BODY_ATTR + "] [data-turn-process-tool-calls] {",
      "  --hana-bar-pad: 10px;",
      "  --hana-bar-edge: 1px;",
      "  margin-inline: calc(-1 * (var(--hana-bar-pad) + var(--hana-bar-edge)));",
      "  width: auto;",
      "  padding: 0 var(--hana-bar-pad);",
      "  background: var(--dsw-alias-bg-layer-1);",
      "  border: var(--hana-bar-edge) solid var(--dsw-alias-border-l1);",
      "  border-radius: 6px;",
      "}",
      "",
    ...
    function apply(ctx) {
      ...
      function reconcile() {
        ...
        applyBrandMark(active === null ? stored : active);
