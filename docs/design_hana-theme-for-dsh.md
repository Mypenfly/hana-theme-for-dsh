...
| E5 | `--dsw-alias-separator-primary`、`--dsw-alias-line-secondary`、`--dsw-alias-label-error`（§3.1、§3.2） | ❌ **三者在本版本均为死令牌**（declare=0 且 consume=0）。写进去不会报错，也不会有任何效果 | `grep -c 'var(--dsw-alias-separator-primary)' <frontend css>` → 0 |
