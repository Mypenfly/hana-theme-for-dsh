...
| E28 | §3.4「极方圆角」只覆盖 `button`/`input`/`select`/`textarea`/`[role=button]`/`[role=tab]`,并以"压方 badges、rails、credential dots 之后放回去要猜哈希类名"为由放弃全局 | ⚠️ **理由成立,但结论建立在一个没有清点过的假设上**。实测(`tools/scan-geometry.mjs`,256 个圆角站点 / 35 个包):**没有一个**圆角读自定义属性,254/256 只挂在构建哈希上——所以"逐站点覆写"这条路本身只有 0.8% 可达,真正的机制只能是**全局钳制**。而那些"圆"按族清点是:状态点 10、圆形控件 21、徽章 13、**图片 1**。也就是说真实损失是**一个站点**,不是一族。参考的实现 `new-warm-paper.css:62-69` 是改 8 个全局圆角令牌(整个应用变方),它的 `--border-width: 0.5px` 属于同一条规则 | 重写为**两个杠杆**:L1 用 DSH 自己的全局杠杆 `--dsw-corner-shape`(它作用在 `*, :before, :after` 上,是参考"一个令牌驱动全体"的同构机制;`superellipse(1.5)` 比正圆更软,所以调向 `round` 就是朝"方"走;DSH 自带的 52 处 `corner-shape: round` 退出自动保住圆形元素);L2 是全局圆角钳制,按参考刻度分四档(2/3/4/6px)。**且必须靠特异性而不是 `!important`**(判据 1):实测 DSH 的特异性天花板是 `(3,0)`(仅 3 条),故钳制写到 `(0,3,1)`,控制层 `(0,3,2)`。`test/verify/seal-check.mjs` 在真实浏览器里验:`seal` 下三档全部 3px、`soft` 下保持 DSH 的 6/6/10px。5 条变异 + 13 条 check.js 判断守住 |
| E29 | §3.2 的 wash 令牌(`interactive-bg-hover` / `-active` / `button-ghost-active-fill` / `sidebar-nav-item-*`)逐格手填 alpha;§3.4 的 `--dsw-specific-tip` 取作"井" | ❌ **两处都错**。① 参考不手挑 hover alpha:它每个主题声明**一条四档阶梯** `--overlay-subtle/-light/-medium/-strong`,且**阶梯不是通用的**——对照色变体每档更重(`midnight-contrast` = .04/.07/.11/.18 vs `midnight` 的 .03/.05/.08/.15)。手填的结果就是漂移:**斑斓成了四套里填充最弱的**(.04/.07 比青夜的 .05/.08 更轻),纸本的 `sidebar-nav-item-hover`(.05)与它自己的 light 档(.04)不一致。② 便笺:方案按 `--jian-note-bg` 映射,但读参考的 `input/InputArea.module.css` 可知它的输入框是 `background: var(--bg-card)`(**八条规则全是**);`--jian-note-bg` 妆点的是右侧便笺编辑器,DSH 无此插槽。而 `specific-tip` 的三个消费者(输入框、队列坞、目标条)全是"落在纸上的卡" | 新增 `tools/derive-wash.mjs`:每个配色一条从**它自己的源主题**转写的阶梯,色相**推导**而非转写(亮色取墨——纸本 `rgb(42,38,34)`、珊瑚 `rgb(26,48,73)`,与参考逐位相同;暗色取纯白,即参考自己立法的极性翻转)。实测并修正 **7 处**漂移。`specific-tip` 改为 `bg-layer-1` 的**镜像**,台账角色从 `plane` 改为 `mirror`;顺带删除一对假断言——`SEPARATION` 里的 "a callout on a card" 在断言一个**不存在的嵌套**(三个消费者都是页面上的兄弟节点)。第四档 `strong` 在 DSH 无消费者,工具要求未消费的档必须在 `UNCONSUMED` 里写明理由,而不是硬塞一个。2 条变异 + 1 个新 suite 守住 |
| E30 | 本仓库的推导工具都要求显式 `--check`;`test/selftest.js` 的 suite 表只接受**一个文件路径** | ⚠️ **一个工具型 gate 无法被变异证明**。`derive-wash` 的证明方式是 `--check` 而非"指向一个文件",而 selftest 的 `spawnSync(process.execPath, [suite])` 只能传一个参数——于是 `suite: 'wash'` 把 `tools/derive-wash.mjs,--check` 当成一个路径,报 `MODULE_NOT_FOUND`,而**基线**用的是另一处代码路径、也要各自修一遍 | suite 表现在允许**带参数**(数组形式),变异运行器与基线运行器都改为展开数组;顺带把基线失败信息里写死的 `.test.js` 去掉。这条记录的价值在于:第一处修完看起来就"通过"了,是基线的分开检查把第二处暴露出来的 |
| E31 | §7 及 `docs/plan-square-geometry.md` §7 的「玻璃层 — `rgba(bg-card, 0.92)`」 | ❌ **只抄了一半机制,而且 `bg-card` 不是四套的答案**。① 参考的浮片是 `background: var(--bg-glass)` **加** `backdrop-filter: blur(12–16px)`(find box 16px、diagnostics badge 12px;最小的「已复制」气泡不模糊)——即 alpha 只是透过率,**模糊是逐片的判断**。② `--bg-glass` 的**颜色没有规律**:11 个源主题里 5 个取 `--bg-card`、5 个取 `--bg`(底色)、`warm-paper` 两头不沾,是**手挑**的。③ 有规律的只有 alpha:9 个 `.92`,2 个 `.94`,而那 2 个**正好是两套对照色变体**。④ 菜单不是玻璃:**11 个源主题里每一个** `.context-menu`/`.slash-menu`/`.mention-menu`/`.model-dropdown`/`.popup`/`.dropdown`/`.floatingPanelInner` 都是不透明的 `--bg-card`,所以 `--dsw-specific-menu` 保持镜像是对的。⑤ 第一次查「谁画 `--dsw-alias-button-tool-bar-fill`」时只扫了各包的 `lib/client.js`,得出「没有插件画它」——**错**:规则在 `dsh-web-frontend` 的 `dist/assets/index-*.css`(`._toolbar_cfgyt_65`)里;正确结论是**规则在、调用点不在**(该哈希在整个前端包里只出现一次,在 class map 导出里) | `tools/derive-glass.mjs`:转抄**取哪张面 + alpha**(纸本/珊瑚取卡,青夜/斑斓取底;斑斓 `.94`),颜色从本配色自己的 `bg-layer-1`/`bg-base` **现取**——第一版连 triple 一起抄,纸本的浮片因此落在 `#FBF7EE` 这个本主题里哪儿都不出现的颜色上。hover 不照 DSH 的 `+0.10`(在 `.92` 上截断成 1.00,合成差约 1/255,是个看不见的 hover),改为**按本配色薄染梯级的 `light` 档往墨色走一步、alpha 不变**,并以 `HOVER_FLOOR` 读**出货值**断言这一步真的看得见。台账新增 `glass` 角色,`test/contrast.test.js #36`(16 条)断言 alpha ∈ [0,.95] 且不透明部分必须是本配色的面或从面往墨走的一步。4 条变异守住 |
| E32 | `docs/plan-square-geometry.md` §7 的「焦点 — `accent-light` + `outline:none`,就是那个橙框」 | ❌ **"杀掉焦点框"是误读**。参考的全局规则(`styles.css:288`,由它自己的 vitest `focus-ring.test.ts` 断言)杀的是**浏览器默认环**,而它**自己又把框画回来**——整个 UI 的按钮基元 `ui/Button.module.css` 就是 `outline: 2px solid var(--accent); outline-offset: 2px`。只抄那条全局 `outline: none` 得到的不是"参考的样子",而是**一个没有焦点提示的界面**。**DSH 的环几何本来就是参考的几何**(都是 1px/2px,offsets ±2px),唯一的差别是**颜色**:31 条非 `none` 的 outline 规则,全部哈希键(可达 0 条)、无一条带 `!important`、分**五**个颜色族——`state-business-primary` ×21、`brand-primary` ×5、`label-tertiary` ×2、`button-info-fill`、`state-warn-label`。珊瑚配色下分别是 `#A8432A`(深朱)与 `#F37E63`(**对底 2.45:1**,低于 WCAG 1.4.11 对非文本指示要求的 3:1)——这就是那个橙框。参考全应用只有**一个**焦点色,而且四套源主题的 `--accent` 在各自底上都过 3:1(3.87/12.52/3.80/6.98) | 新增开关 `data-hana-focus`(默认 `accent`)。规则**只写 `outline-color`**(于是 32 条故意没有环的 DSH 规则不会被凭空加上环),读 `--hana-ring`——纸本/青夜/斑斓的 `brand-primary` **就是**参考的 `--accent`(逐位相同),珊瑚的 `--accent` 是墨蓝、落在 `button-primary-fill` 上,所以那**一个**配色单独按键。特异性 `(0,3,1)`(判据 1 禁 `!important`,而实测天花板是 `(0,3,0)`),另加 `:focus-visible *` 解决**唯一一条**把环画在后代上的 DSH 规则。**没有**全局套用参考最常见的 `border-color: var(--accent)`(29 条),因为它在参考里是逐组件的判断,全局写下去会连承载状态的边一起改色。`tools/derive-focus.mjs` 把参考的 `--accent` 与**规则点名的令牌**绑在一起,`test/verify/focus-check.mjs` 在真实引擎里对 12 个环 ×2 档读回 `outline-color`。8 条变异守住 |
| E33 | §5 的「纸质纹理」与 `docs/plan-square-geometry.md` §7 的「纸纹三层 + 暗色否决 — 缺口补偿层与 `midnight` 否决」 | ❌ **两半都错，而且是反方向的错。** ① **暗色否决根本不是"补偿层的开关"，而是"暗色主题完全不用这个纹理"**：参考在自己的注册表里写着 `paperTextureBlockedThemeIds: ["midnight", "midnight-contrast"]`，`isPaperTextureEffectivelyEnabled = enabled && !blocked`，于是 `body.paper-texture` 这个 class **压根不会被加上**；设置页把开关**禁用**并换成另一句提示（`locales/zh.json`）「黑夜模式不支持纸质纹理，切回浅色主题后会按原设置恢复」——**用户的偏好一个字节都不改**。我们四套配色里**青夜与斑斓一直在跑纸纹**。② **"三层"在 DSH 里三层都够不到**，因为参考的三层落在两个我们关掉了的槽里：它的 ①(surface) 与 ②(card) 都画在**元素自己的 `background-image`**（在内容之下），③ 是 `body::before` + `z-index:-1`（画布之上、面板之下）。而 **DSH 把地面画在 frame 层，不是 `body` 层**——`dsh-client-ui-layout` 的 `AppFrame.module.css` 有 `._1qAH1q_frame { background: var(--dsw-alias-bg-base); height: 100% }`，负 z-index 的层画在"层叠上下文自己的背景之上、流内块级后代的背景之下"，所以**任何 `z-index:-1` 的纹理层都会被 AppFrame 整块盖住**（`.uPhUma_root`、`.eAf-nq_root`、`.isi5dq_panel` 各自再盖一遍自己那一列）。参考**根本不用**"内容之上"这个槽，而那是我们唯一剩下的槽。③ **② 还是空操作**：`background-blend-mode: lighten` 是 `max(卡色, 纹理色)`，而纹理色 (192,182,166) 比参考七个可用主题的**每一张卡**都深，所以结果就是卡色本身——它真正的作用是"纹理永远不许把卡压暗"，而 `soft-light` 在中灰处严格中性，这条保证我们免费就有。④ **③ 补偿的是一次不会发生的变暗**：参考的纹理以 `normal` 合成，把纸本地面从 (245,239,228) 压到 (232.1,225.2,213.0)（掉 12.9/13.8/15.0），那块 `rgba(255,253,247,0.35)` 找回 62%/70%/79%；我们的 ① 是软光中性（#24 逐点证明），搬过来就不是补偿而是一次整体提亮——而且会吃掉珊瑚的卡/底余量（1.0396 → 1.0202，地板是 1.02） | 新增 `tools/derive-grain.mjs`：登记颗粒的参数（baseFrequency/octaves/tile/blend/opacity）并与出货的 SVG 逐项核对；把**出货的**否决函数对**出货的**配色表两个方向都跑一遍；重算 soft-light 的中性、参考文献的整条链（压深 → 找回比例）、以及那块补偿铺到我们四张底上之后还剩多少分离度。**实施的是暗色否决**：新增 `textureAllowed(paletteId)` 按 `scheme` 判（第五套配色由构造被否决，而不是靠有人记得追加 id），`data-hana-texture` 与它相与；设置页的开关在被否决时**显示为关且禁用**并带参考那句提示，**偏好不改写**。纸纹规则同时**按配色键**——两个 body 属性是在同一次 reconcile 里先后写的，中间一次异常就会让暗色配色带着纸纹渲染一帧，按配色键让**画面只依赖一个属性**。9 条 runtime 断言 + 6 条 check.js 判据 + 5 条变异守住 |
| E34 | §5 的规格偏差表:「链接用 `border-bottom`」→「改用 `text-decoration-color`」,理由是「harness 给 markdown 链接画了**透明 2px 下边框 + 负边距**作为放大点击区;覆盖它会缩小点击目标。颜色属于主题,几何不属于」 | ❌ **前半句真,后半句是范畴错误。** 前端确实有那一圈边框(`MarkdownText.module.css`:`text-decoration: none; border-left/right: 3px solid rgb(255 255 255 / 0); border-top/bottom: 2px solid rgb(255 255 255 / 0); margin: ±3px`,注释写着 `Transparent hit-area padding`)——但**命中区是盒子的几何**,给一个**已经存在**的边框**上色**不会把它变窄;**改宽度**才会。那年把"上色"和"改宽度"当成了一件事,于是既没用上参考的做法(它的链接就是 `border-bottom: 1px solid rgba(var(--link-rgb), 0.35)`,hover 时线变实),又留下了一条看起来像已查证的错误结论。顺带:**字距寄存器**那一项也量清了——参考的 76 条 `letter-spacing` 全部挂在类名上,而它的**阅读面只有一条**(`thead th` 0.08em);更关键的是 DSH **自己已经有一份同规律的寄存器**(9 条,.035em 的 kind tag 到 .08em 的官方 wordmark,外加两条**显式清零**),而本主题的边界正是"只有阅读文本变成书",所以那 76 条**不该抄**,不是抄不到 | 链接改回参考的做法:`text-decoration: none` + `border-bottom: 1px solid color-mix(...35%...)`,hover 撤掉 harness 的 `text-decoration: underline` 并把线走到 100%;**只动下边框**,左右/上的透明放大框原样保留,`test/verify/link-check.mjs` 在真实引擎里读回四条边框证明这件事。字距实施阅读面那一对(`--hana-track-label: 0.08em` 给 `th`,`--hana-track-data: 0` 给 `td`——后者今天不改任何像素,是一条写下来的不变量)。新增 **#37**(链接线铺在底上对底 ≥ 1.4,四套实测 1.567/1.796/1.700/2.500,`--verbose` 打印)与 5 条变异。**门自己抓回三处**:#37 第一版把 alpha 抄在测试里,于是"把出货值改成 4%"的变异体没被抓到(现在从出货样式表里解析);hairline 判据只知道 0.5px/2px,**拒绝了一次忠实的移植**,补上"1px、只在下边、只在链接规则上"这条例外,并改为扫去注释的样式表 |
...
- [x] **实现时的三处偏离**（均因读完真实 CSS 后与规格冲突，已就地记录理由）：
  ...
  | 链接用 `border-bottom` | ~~改用 `text-decoration-color`~~ → **已改回 `border-bottom`**（E34） | 原先的理由「覆盖它会缩小点击目标」**是范畴错误**：命中区是**盒子的几何**，给一个已经存在的边框**上色**不会把它变窄——**改宽度**才会。harness 的「透明 2px 下边框」真实存在（`MarkdownText.module.css`：`Transparent hit-area padding`），所以现在**保留左右各 3px 与上边 2px 的透明放大框，只给下边框上色**，宽度取参考自己的 1px |
...
- [x] `data-hana-shape='seal'` 极方圆角 —— 点名控件，默认 `soft` 完全不改圆角（**后被 E28 重写为两个杠杆的全局钳制**，见 `docs/plan-square-geometry.md`）
- [x] **薄染四档 + 便笺面** —— `tools/derive-wash.mjs`；结论与方案相反，见 `docs/plan-square-geometry.md` §7.1–7.2
- [x] **玻璃层** —— `tools/derive-glass.mjs`；方案原文只抄了一半机制，见 E31 / `docs/plan-glass-focus.md` §1
- [x] **焦点墨环** —— `tools/derive-focus.mjs` + `test/verify/focus-check.mjs`；方案原文的"杀掉焦点框"是误读，见 E32 / `docs/plan-glass-focus.md` §2
- [x] **纸纹三层 + 暗色否决** —— `tools/derive-grain.mjs`;三层里能做的一层(暗色否决)做了,另两层逐个给了够不到/不该做的证据,见 E33 / `docs/plan-paper-texture.md`
- [x] **字距寄存器 · 链接细线** —— `docs/plan-tracking-and-links.md`；链接那一项**推翻了本仓库先前记下的一条理由**（E34）
- [x] **`docs/plan-square-geometry.md` §7 七项全部结清**（2026-09-11）
