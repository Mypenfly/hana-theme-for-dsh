...
**`test/contrast.test.js` —— 352 条断言。** 19 对 × 4 套配色、按模式的前景极性、链接可读性区间、纸纹的合成模型、**代码块语法色 9 × 4 + 4 条「色板没有被压平」**、**20 条墨色梯度的形状**、**108 条表面梯级**、**52 条表面色相保持**（§5.9）、**8 条反白标签底板**（§5.9）、**4 条悬停方向**、**16 条层次模型**、**4 条记录在案的链接/错误色间距**、**16 条玻璃层的形状**（见 `docs/plan-glass-focus.md`）。数值**从交付的令牌表里直接读取**，绝不用第二份副本。加 `--verbose` 会打印每一对实测值。
...
**`test/check.js` —— 172 条静态判据。** 禁止哈希选择器、禁止在 `:root` 上声明、禁止在样式表里声明已注册的颜色令牌、禁止无人读取的 `--hana-*` 令牌、禁止用裸字符串调 `settingsScope.bind()`、禁止在 `theme/change` 监听器里同步 `setTheme()`、override 层必须幂等、语法色板必须在配色路径上应用且在 detach 路径上释放、每一个运行时路径都必须在 `files` 里、以及一条 `ctx.effect` 释放链覆盖每一个副作用。新增的判据都做过**变异测试**——把 `applyShiki()` 或 `clearShiki()` 删掉，构建必须失败。
...
**49 条变异对应 49 条行为，全部被抓；**其中数条打在生成的数据文件与绑定表上，数条**复现本项目真实交付过的**值——墨色同值、珊瑚悬停变弱、珊瑚被中性灰定格的卡片、被压平成色块的玻璃浮片。外加一条基线（未变异的源码必须通过涉及的每一个套件），否则「它失败了」不说明任何事。

最后 8 条是为**玻璃层与焦点**写的，其中 2 条**在这两道门里各找出一个真实的洞**：`glass-hover-as-an-alpha-step` 一开始没被抓，是因为 `derive-glass` 在拿自己的推导结果和自己比，而不是和交付值比；`focus-writes-the-outline-shorthand` 一开始没被抓，是因为那条判据**按属性的正确拼法去找规则**——看不见错拼法的判据，抓不住错拼法。两处都已修好，记在 `docs/plan-glass-focus.md` §4。
