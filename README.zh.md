...
| 衬线阅读体 | 开 | 只作用于阅读正文 —— 正文里的链接走参考自己的做法(文字下一条 1px 本色细线,hover 时走到全强度) |
| 衬线字号补偿 | 115%（100–140%） | 见「阅读字号」 |
| 纸质纹理 | 关 | 整屏程序化纸纹 —— **只在亮色配色下生效**（见下） |
| 纹理强度 | 32%（0–60%） | |
| 极方圆角 | 关 | 只把能点名的控件压方 |
| 焦点墨环 | **开** | 键盘焦点框一律改用本配色的强调色，与参考一致 |
| 侧栏印章 | 关 | 用「花」字方印替换侧栏官方标志（本主题唯一一处覆盖自带界面）|

设置页还会显示**设置是否已持久落盘**。如果它说没有，那重启后一定会丢 —— 这条提示存在，是因为一个**静默不落盘**的开关比一个坏掉的开关更糟。

**纸纹在暗色配色下被否决。** 参考不是给暗色主题重绘纹理，而是**根本不加**：它自己的注册表里写着 `paperTextureBlockedThemeIds: ["midnight", "midnight-contrast"]`，开关**禁用**并换成一句提示「黑夜模式不支持纸质纹理，切回浅色主题后会按原设置恢复」。所以青夜与斑斓不带纸纹，开关是**变灰**而不是显示一个不生效的设置，而**你的偏好不会被改写** —— 换回亮色配色，纸纹按你原来设的那样回来。
...
**`test/contrast.test.js` —— 356 条断言。** 19 对 × 4 套配色、按模式的前景极性、链接可读性区间、纸纹的合成模型、**代码块语法色 9 × 4 + 4 条「色板没有被压平」**、**20 条墨色梯度的形状**、**108 条表面梯级**、**52 条表面色相保持**（§5.9）、**8 条反白标签底板**（§5.9）、**4 条悬停方向**、**16 条层次模型**、**4 条记录在案的链接/错误色间距**、**16 条玻璃层的形状**（见 `docs/plan-glass-focus.md`）、**4 条链接线可见度**。数值**从交付的令牌表里直接读取**，绝不用第二份副本。加 `--verbose` 会打印每一对实测值。
...
**`test/check.js` —— 186 条静态判据。** 禁止哈希选择器、禁止在 `:root` 上声明、禁止在样式表里声明已注册的颜色令牌、禁止无人读取的 `--hana-*` 令牌、禁止用裸字符串调 `settingsScope.bind()`、禁止在 `theme/change` 监听器里同步 `setTheme()`、override 层必须幂等、语法色板必须在配色路径上应用且在 detach 路径上释放、每一个运行时路径都必须在 `files` 里、以及一条 `ctx.effect` 释放链覆盖每一个副作用。新增的判据都做过**变异测试**——把 `applyShiki()` 或 `clearShiki()` 删掉，构建必须失败。
...
**`test/runtime.test.js` —— 107 条断言，插件真的被执行。** 上面所有套件读的都是**源码文本**或**交付的表格**；这一套读的是**行为**。这个区别在这里比在别处重要得多，因为「主题静默失败」本来就是运行时属性：一个被 presenter 抹掉的令牌、一个重入的监听器、一次输掉竞态的延迟重应用、一次落不到盘的写入——**没有一个是能从文件文本里看出来的**，而这个项目历史上唯一那次严重缺陷（§5.7 的四层设置持久化连环 bug）**全部由这些构成**。那次 bug 得出的教训是「① 显式契约 + ② 可见的持久化状态 + ③ 判据」；① 和 ② 早就上了，**③ 从来没上**。
...
**54 条变异对应 54 条行为，全部被抓；**其中数条打在生成的数据文件与绑定表上，数条**复现本项目真实交付过的**值——墨色同值、珊瑚悬停变弱、珊瑚被中性灰定格的卡片、被压平成色块的玻璃浮片。外加一条基线（未变异的源码必须通过涉及的每一个套件），否则「它失败了」不说明任何事。

最近的几条是为**玻璃层、焦点、纸纹与链接**写的，其中 3 条**在各自的门里找出了真实的洞**：`glass-hover-as-an-alpha-step` 一开始没被抓，是因为 `derive-glass` 在拿自己的推导结果和自己比，而不是和交付值比；`focus-writes-the-outline-shorthand` 一开始没被抓，是因为那条判据**按属性的正确拼法去找规则**——看不见错拼法的判据，抓不住错拼法。两处都已修好，记在 `docs/plan-glass-focus.md` §4。

变异现在可以指定**目标文件**与**要跑的套件**（`test/runtime.test.js` / `tokens.test.js` / `contrast.test.js` / `check.js` / 各 `--check` 推导工具），所以 `test/load-client.js`、`test/tokens.test.js` 与 `test/check.js` 也接受 `HANA_BUNDLE` / `HANA_ALLOWLIST` / `HANA_GEOMETRY` 指向副本——真实文件永远不被改写。这一步是必要的：一个从没被证明能对**已知坏输入**失败的判据只是装饰。
