 ...
 *
 * Recorded as a measurement rather than a sentence, because "the ledger covers
 * every colour surface" is the kind of claim that is read as stronger than it
 * is. Two surfaces exist that no theme registered through the theme service can
 * reach, and both are structural rather than oversights:
 *
 *   1. DSH Desktop's `native-ui` documents — the profile switcher, the first-run
 *      wizard, profile creation, crash recovery and the desktop dialog. They are
 *      SEPARATE DOCUMENTS with no client-plugin host, and `desktop-dialog.html`
 *      declares `style-src 'self'`, which forbids the inline <style> element
 *      that is this theme's only remaining channel. Their chrome is Tailwind /
 *      shadcn, whose vocabulary (`--background`, `--card`, `--gray1..12`, the
 *      `--tw-*` set) addresses nothing the theme service writes.
 *   2. Separately installed community plugins that invent their own `--dsw-*`
 *      names. A theme cannot be responsible for a contract nobody publishes;
 *      tools/refresh-allowlist.mjs counts them under `outsideScope` instead.
 *
 * Both are counted from the filesystem so the boundary lands in the ledger as a
 * number. Neither is a defect to fix.
 */
function nativeUiSheets() {
  const store = '/nix/store'
  if (!existsSync(store)) return []
  const out = new Set()
  for (const name of readdirSync(store)) {
    if (!/^[a-z0-9]{32}-dsh-desktop/.test(name)) continue
    for (const tail of ['lib/dsh-desktop/resources/app', 'resources/app']) {
      const app = join(store, name, tail)
      ...
      const walk = (dir, depth) => {
        ...
        for (const e of entries) {
          ...
          /* Deduped by REAL path: more than one store entry can carry the same
             app tree, and counting it twice would double the boundary figure
             that exists precisely to be trustworthy. */
          if (e.name.endsWith('.css')) { try { out.add(realpathSync(p)) } catch { out.add(p) } }
        }
      }
      walk(app, 0)
    }
...
/** Colour-carrying property names in those sheets that no theme could address. */
function unreachableNames(sheets) {
  const names = new Set()
  for (const file of sheets) {
    let css
    try { css = readFileSync(file, 'utf8') } catch { continue }
    for (const [name] of colourDeclarations(css)) {
      if (/^--(dsw|dsh|dsl|shiki|hana|ds-font)/.test(name)) continue
      names.add(name)
    }
  }
  return [...names].sort()
...
const payload = {
  ...
  source: {
    ...
    notCovered: {
      what:
        "DSH Desktop's native-ui documents (separate documents, no plugin host, and " +
        "desktop-dialog.html declares style-src 'self' so even an injected <style> is " +
        'forbidden) and any separately installed community plugin',
      /* Reported as LOGICAL sheets. The store carries the desktop shell
         vendored inside the desktop app as well as on its own, so the raw file
         count is double and would read as twice as much unreachable UI as there
         actually is. */
      /* Counted as distinct sheets WITHIN ONE COPY, keyed by path relative to
         `resources/app`, and with the number of copies beside it. Deduping by
         basename instead would collapse same-named modules from different
         directories and under-report; not deduping at all would double it,
         because the store carries the desktop shell vendored inside the desktop
         app as well as on its own. */
      desktopCssFiles: new Set(nativeUiSheets().map((p) => p.slice(p.indexOf('/resources/app/')))).size,
      desktopCopies: new Set(nativeUiSheets().map((p) => p.slice(0, p.indexOf('/resources/app/')))).size,
      unreachableNames: unreachableNames(nativeUiSheets()).length,
      note:
        'Counted, not classified: those surfaces are structurally outside the theme service, ' +
        'so a decision per name would imply a control this theme does not have. Named roots here ' +
        'are the web profile; the desktop profile resolves the same contracts through the nix ' +
        'store copies that tools/refresh-allowlist.mjs searches.',
    },
  },
